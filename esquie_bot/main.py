...
@tree.command(name="audio", description="Generate AI speech from text")
@discord.app_commands.describe(text="The text to convert to speech", voice="Choose the AI voice to use")
@discord.app_commands.choices(voice=[
    discord.app_commands.Choice(name="Alloy (Balanced)", value="alloy"),
    discord.app_commands.Choice(name="Echo (Male)", value="echo"),
    discord.app_commands.Choice(name="Fable (British Female)", value="fable"),
    discord.app_commands.Choice(name="Onyx (Male)", value="onyx"),
    discord.app_commands.Choice(name="Nova (Young Female)", value="nova"),
    discord.app_commands.Choice(name="Shimmer (Female)", value="shimmer"),
    discord.app_commands.Choice(name="Coral (Female)", value="coral"),
    discord.app_commands.Choice(name="Verse (Male)", value="verse"),
    discord.app_commands.Choice(name="Ballad (Female)", value="ballad"),
    discord.app_commands.Choice(name="Ash (Male)", value="ash"),
    discord.app_commands.Choice(name="Sage (Male)", value="sage"),
    discord.app_commands.Choice(name="Amuch (Male)", value="amuch"),
    discord.app_commands.Choice(name="Dan (Male)", value="dan")
])
async def audio_command(interaction: discord.Interaction, text: str, voice: str = "alloy"):
    """Generate AI speech using Pollinations.AI text-to-audio API."""
    try:
        await interaction.response.defer()  # Defer response since audio generation takes time
        
        log(f"[AUDIO_CMD] User {interaction.user.name} requested audio with text: '{text}' using voice: '{voice}'")
        ...
        log(f"[AUDIO_CMD] Making request to {url} with params: {params}")
        ...
        # Create a Discord file attachment
        filename = f"generated_audio_{int(datetime.now().timestamp())}.mp3"
        file = discord.File(audio_data, filename=filename)
        
        # Send the audio as an embed with the text
        embed = discord.Embed(
        ...
        await interaction.followup.send(embed=embed, file=file)
        log(f"[AUDIO_CMD] Successfully sent generated audio to {interaction.user.name}")
        
    except requests.Timeout:
        log("[AUDIO_CMD] Request timed out")
        await interaction.followup.send("⏰ Sorry, audio generation is taking too long. Please try again with shorter text!")
    except requests.RequestException as e:
        log(f"[AUDIO_CMD] Request failed: {e}")
        await interaction.followup.send("❌ Sorry, I'm having trouble generating the audio right now. Please try again later!")
    except Exception as e:
        log(f"[AUDIO_CMD] Unexpected error: {e}")
        await interaction.followup.send("💥 Oops! Something went wrong while generating your audio. Please try again!")
